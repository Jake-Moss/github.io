...
- ="contents":= recursively publish any file ending in =.org= (except =index.org=) in the =base-directory= to the =public-directory= with the =org-html-publish-to-html= function.
