...
All pages here (including the mono-page) are generated from a set of =.org= files, published via the =org-html-publish-to-html= function, smeared with a little =CSS= [[https://latex.vercel.app][that isn't mine]], then thrown up on a [[https://github.com/Jake-Moss/github.io][github.io]] page. All-in-all it was about an evenings work to learn how the =org-publish= system worked, experiment, and polish to just what I had in mind.

* What about the so-called publishing script being an article?
Yeah so this article isn't /really/ the publishing script. Specifically the =bootstrappaing.org= file is /tangled/ to =publish.el=, which is then executed.

But it didn't use to be like that. To bootstrap the page I wrote a bog-standard =.el= file and could be loaded and executed from the command-line to publish the page during testing,
#+begin_src sh
...
I use a few custom variables as well. =inline-html-publish-index-subpages= is a bit interesting, because I wanted to generate a mono-page as well as the individual pages I need to have a custom publishing function and I need to store the subpages for addition to the mono-page.
The =-marker= variables are used to mark where some the table of contents and subpages should be inserted. It's a bit of a hack I know I know but it works just fine.
#+begin_src emacs-lisp
...
The real meat of the =org-publish= system is the =org-publish-project-alist= variable, it defines what files should be published and how with some knobs to fiddle with. This is a pretty boring one, we only have
...
- ="index":= is a little more interesting. It's very similar to ="contens"= but use my =inline-html-publish-index= function to publish the =index.org= file.
#+begin_src emacs-lisp
...
The previously mentioned =inline-html-publish= function works by getting the to-be-published file and exporting it as =HTML= into a temp buffer, adding some =HTML= to the top via a format string, then plopping that plus some meta data into the =inline-html-publish-index-subpages= variable.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  Merges the `:inline-plist' with the plist of `:inline-components' in PLIST"
    ...
    (let* ((org-inhibit-startup t)
      ...
      (unless visiting (kill-buffer work-buffer))))
#+end_src
The =:inline-components= use the =inline-html-publish= function which populates the =inline-html-publish-index-subpages= variable. That's sorted and the timestamps tripped out. It then finds the markers within the html buffer and inserts the relevant content.

And that's it.

I think it works quite well and just shows how extensive the =org-publish= system is.

** Wait wait where did those markers come from?
Oh yeah, they are just embedded as exported =HTML= blocks in the =index.org= file. Here's what that looks like
